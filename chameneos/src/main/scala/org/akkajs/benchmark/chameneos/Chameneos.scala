package org.akkajs.benchmark.chameneos

import akka.actor._

import org.scalajs.benchmark.Done

object Chameneos extends org.scalajs.benchmark.Benchmark {

  override def prefix = "Chameneos"

  def run(system: ActorSystem, ref: ActorRef) = {
    system.actorOf(Props(new Actor {
      context.actorOf(Props(new Mall(100, 4))) ! Start(100, 4)
      def receive = {
        case "done" =>
          ref ! Done
      }
